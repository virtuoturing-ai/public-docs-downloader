...
def cookie_file() -> Path:
    return runtime_dir() / COOKIE_FILE_NAME


def save_cookie_snapshot(cookies: List[dict]) -> None:
    p = cookie_file()
    tmp = p.with_suffix(p.suffix + ".tmp")
    try:
        tmp.write_text(json.dumps(cookies or [], ensure_ascii=False, indent=2), encoding="utf-8")
        tmp.replace(p)
    except Exception:
        pass


def load_cookie_snapshot() -> List[dict]:
    p = cookie_file()
    if not p.exists():
        return []
    try:
...
class DownloaderWorker(QObject):
    ...
    def _is_auth_reason(reason: str) -> bool:
        ...
        return (
            "auth_required" in r
            or "authorization/age gate" in r
            or "http 401" in r
            or "http 403" in r
        )

    def _sync_cookie_store_from_session(self, session) -> int:
        snap: List[dict] = []
        try:
        ...
        changed = self.cookie_store.merge(snap)
        if changed:
            save_cookie_snapshot(self.cookie_store.get_all())
        return changed
    ...
    def _request_page_html(self, session, url: str, retries: int = PAGE_RETRIES) -> Tuple[Optional[str], str]:
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                if code in (401, 403):
                    self._sync_cookie_store_from_session(session)
    ...
    def _download_pdf(self, session, url: str, target: Path, referer: str) -> DownloadResult:
        ...
        for attempt in range(1, FILE_RETRIES + 1):
            ...
            try:
                ...
                if code in (401, 403):
                    self._sync_cookie_store_from_session(session)
                    return DownloadResult(False, "auth_required", permanent_skip=False)
    ...
    def _process_dataset(self, ds: int) -> None:
        ...
        try:
            while (not self._is_stopped()) and (page <= max_pages):
                ...
                if html is None:
                    if self._is_auth_reason(reason):
                        self.log.emit(f"[DS{ds}] Auth gate detected at page {page}/{max_pages}. Pausing for re-auth.")
                ...
                if not links:
                    ...
                    for pdf_url in links:
                        ...
                        if not self._claim_url(pdf_url):
                            continue
                        ...
                        elif self._is_auth_reason(res.reason):
                            self.log.emit(f"[DS{ds}] Auth gate detected during file download. Pausing for re-auth.")
                            self._trigger_auth_required(page_url, ds, page)
                            self._checkpoint("auth_required", fail_reason=res.reason, force=True)
                            return
                        else:
                            self._inc_result(failed=1)
                            self._mark_dataset(ds, processed_inc=1)
    ...
    def run(self) -> None:
        ...
        self.log.emit(f"Parallel mode: processing all datasets simultaneously (workers={PARALLEL_DATASET_WORKERS}).")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        initial_cookies = load_cookie_snapshot()
        if initial_cookies:
            self.cookie_store.set_all(initial_cookies)
        self._initial_cookie_count = self.cookie_store.count()
        ...
        self._last_auth_url: str = ROOT_URL
        self._last_auth_ds: int = DATASET_MIN
        self._last_auth_pg: int = 1
        self._auto_resume_in_progress: bool = False

        self._build_ui()
        self._apply_style()
        if self._initial_cookie_count > 0:
            self.append_log(f"Loaded persisted cookies: {self._initial_cookie_count}.")
    ...
    def _resolve_target_url_from_state(self, resume_state: Optional[dict]) -> str:
        if not resume_state:
            return ROOT_URL
        ds = int(resume_state.get("dataset_idx") or DATASET_MIN)
        pg = int(resume_state.get("page_idx") or 1)
        ds = max(DATASET_MIN, min(DATASET_MAX, ds))
        pg = max(1, pg)
        return DownloaderWorker._page_url(ds, pg)
    ...
    def _validate_current_session(self, target_url: str) -> bool:
        if self.cookie_store.count() <= 0:
            return False
        ...
        try:
            ...
            self._apply_cookies_to_session(s)
            resp = s.get(target_url, timeout=30, allow_redirects=True)
            code = int(resp.status_code)
            html = resp.text or ""
            try:
                resp.close()
            ...
            if code in (401, 403) or code >= 400:
                return False

            links = DownloaderWorker._extract_pdf_links(html, target_url)
            if links:
                return True
            ...
            if any(m in low for m in blockers):
                return False

            return ("doj-disclosures" in low) or ("data-set-" in low)
        except Exception:
            return False
        finally:
            try:
    ...
    def _launch_worker(self, resume_state: Optional[dict]) -> None:
        self.progress.setValue(0)
        self.progress_total.setValue(0)
    ...
    def _auto_resume_after_auth(self):
        if self._auto_resume_in_progress:
            return
        self._auto_resume_in_progress = True

        try:
            resume_state = load_state()
            if not resume_state:
                self.append_log("Checkpoint not found after auth gate. Click Start to begin again.")
                return

            target_url = self._last_auth_url or self._resolve_target_url_from_state(resume_state)
            self.append_log(f"Trying automatic session validation on: {target_url}")

            if self._validate_current_session(target_url):
                self.append_log("Stored cookies are still valid. Resuming automatically.")
                self._launch_worker(resume_state=resume_state)
                return

            self.append_log("Automatic validation failed. Opening browser for re-confirmation...")
            if not self._open_age_dialog(target_url=target_url):
                self.append_log("Re-confirmation not completed. Click Start to resume when ready.")
                return

            self.append_log("Re-confirmation captured. Resuming automatically from checkpoint.")
            self._launch_worker(resume_state=resume_state)

        finally:
            self._auto_resume_in_progress = False
    ...
    def start_clicked(self):
        ...
        target_url = self._resolve_target_url_from_state(resume_state)

        if self._validate_current_session(target_url):
            self.append_log("Stored cookies validated. Starting without manual confirmation.")
            self._launch_worker(resume_state=resume_state)
            return
    ...
    def on_auth_required(self, url: str, ds: int, pg: int):
        ds = max(DATASET_MIN, min(DATASET_MAX, int(ds)))
        pg = max(1, int(pg))
        self._last_auth_ds = ds
        self._last_auth_pg = pg
        self._last_auth_url = (url or DownloaderWorker._page_url(ds, pg))
        self.append_log(
            f"Session likely expired at Data Set {ds}, page {pg}. "
            "Will attempt automatic re-validation and resume."
        )
    ...
    def on_finished(self, downloaded: int, skipped: int, failed: int, status: str):
        ...
        if status == "completed":
            ...
            detail = (
                ...
                "Automatic re-validation will start now."
            )
        else:
        ...
        if status == "auth_required":
            QTimer.singleShot(0, self._auto_resume_after_auth)
            return
